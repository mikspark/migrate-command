        ...
        if ($this->_modulePaths === null) {
            ...
            foreach (Yii::app()->modules as $module => $config) {
                if (is_array($config)) {
                    ...
                    $paths[$config] = 'application.modules.' . $config . '.' . ltrim($this->migrationSubPath, '.');
                }

                $sorter->add($module, Yii::app()->getModule($module)->getDependencies());
            }

            $this->_modulePaths = [];
            foreach ($sorter->sort() as $module) {
                $this->_modulePaths[$module] = $paths[$module];
            }
        }
